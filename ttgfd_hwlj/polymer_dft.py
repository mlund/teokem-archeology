...
def write_input_file(args, output_path="input.tsph"):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"  {args.dmm:22.15f}       {args.dms:22.15f}\n"
...
def main():
    ...
    # Physical parameters
    parser.add_argument(
        "--bdm", type=float, default=1.0e-2, help="Monomer bulk density"
    )
    parser.add_argument("--bdtot", type=float, default=1.0, help="Total bulk density")
    parser.add_argument(
        "--nmon", type=int, default=151, help="Number of monomers per polymer chain"
    )

    # Grid parameters
    parser.add_argument(
        "--dz", type=float, default=0.25, help="Grid spacing in z direction"
    )
    parser.add_argument(
        "--drho", type=float, default=0.25, help="Grid spacing in radial direction"
    )
    parser.add_argument(
        "--dphi",
        type=float,
    ...
    # Geometry parameters
    parser.add_argument("--Rcoll", type=float, default=10.0, help="Colloid radius")
    parser.add_argument(
        "--zc1", type=float, default=35.0, help="Position of first colloid center"
    )
    parser.add_argument(
        "--collsep", type=float, default=20.0, help="Separation between colloid centers"
    )
    parser.add_argument(
        "--Rcyl", type=float, default=35.0, help="Cylinder radius (system boundary)"
    )

    # Molecular parameters
    parser.add_argument("--bl", type=float, default=1.0, help="Bond length")
    parser.add_argument(
        "--dhs", type=float, default=1.0, help="Hard sphere diameter (monomer)"
    )
    parser.add_argument(
        "--epslj",
        type=float,
    ...
    parser.add_argument(
        "--ioimaxm", type=int, default=5000, help="Maximum number of iterations"
    )
    parser.add_argument(
        "--dmm", type=float, default=0.9, help="Density mixing parameter (monomer)"
    )
    parser.add_argument(
        "--dms", type=float, default=0.5, help="Density mixing parameter (solvent)"
    )
    parser.add_argument(
        "--kread", action="store_true", help="Read initial guess from file fcdfil"
    )
