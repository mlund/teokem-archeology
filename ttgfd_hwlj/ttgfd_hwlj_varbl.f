...
c     rhop loop
      ...
      do iphi = 1,npphi
      phi = phi+dpphi
c      s = dsqrt(useful-trmix*dcos(phi))
      s2 = useful-trmix*dcos(phi)
      if (s2.gt.dhs2) then
c      s = dsqrt(s2)
c      pint = dexp(-tkappa*s)/s+pint
      pint = rlj/s2**6-alj/s2**3+pint
c      uuu = rlj/s2**6-alj/s2**3
c      if (uuu.lt.50.d0) pint = uuu+pint
c      if (uuu.lt.16.d0) pint = 0.01d0+pint
c      pint = 0.01d0+pint
c      pint = uuu+pint
      endif
      enddo
 ...
 100  continue
      ...
      do iphi = 1,nphi
      phi = phi+dphi
c     plus eller minus spelar ingen roll foer integralens vaerde
 ...
 523  continue
c     rho0 svepet faerdigt!
      enddo
c     dags foer nytt z0 vaerde!
 245  continue
c     dags foer ny monomer laengs kedjan
      do iz = istp1,ibl
      do kz = 1,mxrho+kbl
...
c      if (tfdm.gt.(10.*fdm)) then
c      tfdm = 10.d0*fdm
c      tfem = 10.d0*fem(j,i)
c      endif
...
c     net force from rho iterations:
...
c      zpst = z-1.d0-dz
      ...
      delz2 = (zp-z)**2
      sumrhop = 0.d0
c      rhopmax = dsqrt(dabs(1.d0-delz2))
